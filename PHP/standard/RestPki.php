...
class Color {
    ...
    public function __construct() {
        ...
        else if (sizeof($args) == 2 && is_string($args[0]) && strlen($args[0]) == 7) { // Case ("#RRGGBB", a)
            $this->red = hexdec(substr($args[0], 1, 2));
            $this->green = hexdec(substr($args[0], 3, 2));
            $this->blue = hexdec(substr($args[0], 5, 2));
            $this->alpha = $args[1];
        }
        else if (sizeof($args) == 3) { // Case (r, g, b)
            $this->red = $args[0];
            $this->green = $args[1];
            $this->blue = $args[2];
            $this->alpha = 100;
        }
        else if (sizeof($args) == 4) { // Case (r, g, b, a)
            $this->red = $args[0];
            $this->green = $args[1];
            $this->blue = $args[2];
            $this->alpha = $args[3];
...
class PdfMarkElement {
    ...
    public function __construct() {
        ...
        else if (sizeof($args) == 2) { // Case (elementType, relativeContainer)
            $this->rotation = 0;
            $this->elementType = $args[0];
...
class PdfMarkTextElement extends PdfMarkElement {
    ...
    public function __construct() {
        ...
        else if (sizeof($args) == 2) { // Case (relativeContainer, sections)
...
class PdfMarkImageElement extends PdfMarkElement {
    ...
    public function __construct() {
        ...
        else if (sizeof($args) == 2) { // Case (relativeContainer, image)
            $this->image = $args[1];
...
class PdfTextSection {
    ...
    public function __construct() {
        ...
        else if (sizeof($args) == 2) { // Case (text, color)
            $this->text = $args[0];
            $this->color = $args[1];
            $this->fontSize = null;
        }
        else if (sizeof($args) == 3) { // Case (text, color, fontSize)
            $this->text = $args[0];
            $this->color = $args[1];
...
class PdfMarkImage {
    ...
    public function __construct() {
        ...
        else if (sizeof($args) == 2) { // Case (imageContent, mimeType)
            $this->resource = new ResourceContentOrReference();
