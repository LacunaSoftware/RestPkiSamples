...
(function ($) {
	...
	$.Promise.prototype._invokeSuccess = function (result, delay) {
		...
		var callbackInvoke = function () {
			callback(result);
		};
		if (delay) {
			var self = this;
			setTimeout(function () { self._apply(callbackInvoke); }, delay);
		} else {
			this._apply(callbackInvoke);
		}
	};
	...
	$._installUrl = 'https://webpki.lacunasoftware.com/Install';

	$._chromeLatestExtensionVersion = '1.3.1';
	$._chromeLatestNativeVersion = '1.5.3';
	...
	*/
	...
	$.listCertificates = function (args) {
		...
		$._requestHandler.sendCommand(context, 'listCertificates', null);
		return context.promise;
	};
	...
	if ($._requestHandler === undefined) {
		...
		if ($.detectedBrowser.indexOf("Chrome") >= 0) {
			...
			$._requestHandler = new function () {
				...
				var sendCommand = function (context, command, request) {
					var requestId = registerPromise(context.promise);
					var message = {
						requestId: requestId,
				...
				var checkExtension = function (context, tryCount) {
					...
					subPromise.success(function (version) {
						if ($._compareVersions(version, $._chromeLatestExtensionVersion) < 0) {
							context.promise._invokeSuccess({
								isInstalled: false,
								status: $.installationStates.OUTDATED,
								message: 'The Web PKI extension is outdated (installed version: ' + version + ', latest version: ' + $._chromeLatestExtensionVersion + ')',
								browserSpecificStatus: $._chromeInstallationStates.EXTENSION_OUTDATED
							});
						} else {
							checkNative(context); // if getExtensionVersion passes, we call checkNative with the original context, that is, forwarding the angularScope to checkNative
						}
					});
					// we must not simply do "subPromise.error(context.promise.errorCallback);", otherwise the error callback would be called outside of $scope.$apply if angularScope is set
				...
				var checkNative = function (context) {
					var subPromise = new $.Promise(null);
					subPromise.success(function (version) {
						if ($._compareVersions(version, $._chromeLatestNativeVersion) < 0) {
							context.promise._invokeSuccess({
								isInstalled: false,
								status: $.installationStates.OUTDATED,
								message: 'The Web PKI native component is outdated (installed version: ' + version + ', latest version: ' + $._chromeLatestNativeVersion + ')',
					...
					subPromise.error(function (message, error, origin) {
						if (message.indexOf('Did not receive response from native component') >= 0) {
							context.promise._invokeSuccess({
								isInstalled: false,
								status: $.installationStates.NOT_INSTALLED,
								message: 'The Web PKI native component is not installed',
								browserSpecificStatus: $._chromeInstallationStates.NATIVE_NOT_INSTALLED
							});
						} else {
							context.promise._invokeError(message, error, origin);
						}
				...
				var onResponseReceived = function (data) {
					...
					var promise = promises[result.requestId];
					delete promises[result.requestId];
					if (result.success) {
						promise._invokeSuccess(result.response);
					} else {
						promise._invokeError(result.exception.message, result.exception.complete, result.exception.origin);
					}
				};
			...
			$._requestHandler = new function () {
				...
				var poll = function () {
					...
					if (lacunaWebPKIExtension) {

						var resultsJson = lacunaWebPKIExtension.GetAvailableResults();
						if (resultsJson == null) {
						...
						for (var requestId in pendingRequests) {
							...
							if (pendingRequest.sendFailed) {
								...
								if (result != null) {
									...
									pendingRequest.promise._invokeError('The operation has timed out', 'The operation has timed out', 'jslib');
									removePendingRequest = true;
								}
							}
						...
						for (var i = 0; i < requestIdsToRemove.length; i++) {
							var requestId = requestIdsToRemove[i];
				...
				var sendCommand = function (context, command, request) {
					if (lacunaWebPKIExtension) {
						...
						try {
							var success = lacunaWebPKIExtension.SendCommand(JSON.stringify(message));
							if (success === false) {
						...
						if (sendCommandError) {
							context.promise._invokeError('Failed to send command to add-on', sendCommandError, 'jslib', 200);
							pendingRequests[requestId].sendFailed = true;
						}
					} else {
						context.promise._invokeError('Add-on not detected', 'Add-on not detected', 'jslib', 200);
					}
				};
			...
			$._requestHandler = new function () {
				...
				var sendCommand = function (context, command, request) {
					context.promise._invokeError('Browser not supported', 'Browser not supported', 'jslib', 200);
				};
