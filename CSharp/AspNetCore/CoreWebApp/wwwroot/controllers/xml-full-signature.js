'use strict';
app.controller('xmlFullSignatureController', ['$scope', '$http', 'blockUI', 'util', function ($scope, $http, blockUI, util) {
    ...
    $scope.sign = function () {
        ...
        $http.post('Api/XmlFullSignature').then(function (response) {
    ...
    var onSignCompleted = function (token) {
        $http.post('Api/XmlFullSignature/' + token).then(function (response) {
            blockUI.stop();
