...
(function ($) {
	...
	.error(function (message, error, origin, code) {
		// The error callback always receives 4 arguments:
		// - message: a user-friendly message describing the error that occurred
		// - error: a detailed string containing as much information about the error as possible, for instance the stack trace. This is a good value to be logged, not to be shown to the user.
		// - origin: a string denoting where the error originated. This should also not be shown to the user, but rather logged for diagnostic purposes.
	...
	$._extensionRequiredVersion = '2.11.3';
	$._chromeNativeWinRequiredVersion = '2.6.1';
	$._chromeNativeLinuxRequiredVersion = '2.7.3';
	$._chromeNativeMacRequiredVersion = '2.7.3';
	$._ieAddonRequiredVersion = '2.4.1';
    // ------------------------------------------------
    ...
    $._apiMap.nativeWin[$.apiVersions.v1_4] = '2.6.1';
    ...
    $._apiMap.ieAddon[$.apiVersions.v1_4] = '2.4.1';
    ...
    $._apiMap.nativeLinux[$.apiVersions.v1_4] = '2.7.3';
    ...
    $._apiMap.nativeMac[$.apiVersions.v1_4] = '2.7.3';
    ...
    $._apiMap.extension[$.apiVersions.v1_4] = '2.11.3';
	...
	*/
	...
	$.redirectToInstallPage = function () {
		document.location.href = $._installUrl + (this.brand || '') + '?returnUrl=' + encodeURIComponent(document.URL) + '&jslib=2.8.0';
	};
	...
	$.signFullXml = function (args) {
		...
		return $._signXmlCommon(args, request);
	};

	$.signXmlElement = function (args) {
		var request = {
			signerType: 'xmlElement',
			toSignElementId: args.toSignElementId,
			toSignElementsIds: args.toSignElementsIds,
			toSignElementsXPath: args.toSignElementsXPath,
			idResolutionTable: args.idResolutionTable
		};

		return $._signXmlCommon(args, request);
	};

	$._signXmlCommon = function (args, request) {
		var context = this._createContext(args);

		// base
		request.fileId = args.fileId;
		request.certificateThumbprint = args.certificateThumbprint;
		request.output = {
			mode: args.output.mode,
			folderId: args.output.folderId,
			dialogTitle: args.output.dialogTitle,
		...
		request.trustArbitrators = args.trustArbitrators;
		request.clearPolicyTrustArbitrators = args.clearPolicyTrustArbitrators;
	...
	$.openXades = function (args) {
	    ...
	    $._requestHandler.sendCommand(context, 'openXades', request);
	    return context.promise;
	};
