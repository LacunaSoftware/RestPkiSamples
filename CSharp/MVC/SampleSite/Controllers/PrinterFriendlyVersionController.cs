...
{
    public class PrinterFriendlyVersionController : Controller
    {
		// ##################################################################################################
		...
		private const string VerificationSite = "http://localhost:55611/";

		// Format of the verification link, with "{0}" as the verification code placeholder.
		private const string VerificationLinkFormat = "http://localhost:55611/Check?code={0}";
		...
		// GET: PrinterFriendlyVersion/File/{id}
		public ActionResult File(string id)
        {
			...
			try {
				fileCntent = StorageMock.Read(id, out var extension);
			} catch (FileNotFoundException) {
				return HttpNotFound();
			}

			// Check if doc already has a verification code registered on storage.
			var verificationCode = StorageMock.GetVerificationCode(id);
			if (verificationCode == null) {
				// If not, generate a code an register it.
				verificationCode = Util.GenerateVerificationCode();
				StorageMock.SetVerificationCode(id, verificationCode);
			}

			// Generate the printer-friendly version.
			var pfvContent = generatePrinterFriendlyVersion(fileCntent, verificationCode);
