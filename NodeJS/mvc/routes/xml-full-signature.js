 ...
 *
 * This route initiates a full XML signature using REST PKI and renders the
 * signature page.
 */
router.get('/', function(req, res, next) {
   ...
   var restRequest = {
      ...
      xml: new Buffer(util.getSampleXml()).toString('base64'),
      ...
      // Set the location on which to insert the signature node. If the location
      // is not specified, the signature will appended to the root element
      // (which is most usual with enveloped signatures).
   ...
   // signature.
   request.post(util.endpoint + 'Api/XmlSignatures/FullXmlSignature', {
      ...
      if (restPki.checkResponse(err, restRes, body, next)) {
         ...
         // component (see view 'xml-full-signature') and also to complete the
 ...
 *
 * This route receives the form submission from the view 'xml-full-signature'.
