...
def index():
    ...
    try:
        ...
        # This should not be mistaken with the API access token.
    ...
    # order to retry authenticating it is necessary to get a new token. This can
    # be a problem if the user uses the back button of the browser, since the
    # browser might show a cached page that we rendered previously, with a now
    # stale token. To prevent this from happening, we force page expiration
...
@authentication.route('/action', methods=['POST'])
def action():
    """

    This view function receives the form submission from the template. We'll
    call REST PKI to validate.
    ...
    # authentication process. The call yields a ValidationResults object, which
    # denotes whether the authentication was successful or not (we'll use it to
    # render the page accordingly, see below).
    try:
    ...
    return render_template('authentication/action.html', valid=vr.is_valid,
                           user_cert=user_cert, vr_html=vr_html)
