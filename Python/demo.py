...
if restPkiAccessToken == 'PLACE YOUR API ACCESS TOKEN HERE':
	raise Exception('The API access token was not set! Hint: to run this sample you must generate an API access token on the REST PKI website and paste it on the file demo.py')
...
def auth():
	...
	# method on the route /authentication-action. This should not be mistaken with the API access token.
	try:
...
def auth_action():
	...
	# Call the complete_with_webpki() method with the token, which finalizes the authentication process. The call yields a
	# ValidationResults object, which denotes whether the authentication was successful or not (we'll use it to render the page
	...
	try:
		...
		if vr.is_valid():
			...
			# method auth() and that the user is indeed the certificate's subject. Now, you'd typically query your
...
def pades_signature(userfile=None):
	...
	try:
		...
		# signWithRestPki() method on the Web PKI component (see lacuna-web-pki-client.js javascript) and also to complete the signature after
		# the form is submitted (see method pades_signature_action())). This should not be mistaken with the API access token.
	...
	response = make_response(render_template('pades-signature.html', token=token, pdfPath=pdfPath))
	response.headers = get_expired_page_headers()
	return response
