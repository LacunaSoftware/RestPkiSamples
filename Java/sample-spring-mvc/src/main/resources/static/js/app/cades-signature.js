/*
 * This javascript file contains the client-side logic for the CAdES signature example. Other parts can be found at:
 * - HTML: Views/Home/CadesSignature.cshtml
 * - Server-side logic: Api/CadesSignatureController
 *
...
function onSignatureStartCompleted(data, textStatus, jqXHR) {
	...
	// Call signData() on the LacunaWebPKI object
	pki.signData({
		thumbprint: selectedCertThumbprint,      // selected certificate's thumbprint
		data: data.toSign,                       // "to-sign-bytes" received from the server
...
function onSignDataCompleted(signature) {
	...
	$.ajax({
		method: 'POST',
		url: '/Api/CadesSignature/Complete',
		data: JSON.stringify({
			processId: processId, // signature process ID, returned by the server along with the "to-sign-bytes"
