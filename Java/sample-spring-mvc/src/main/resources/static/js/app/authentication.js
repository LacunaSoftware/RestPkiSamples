...
function signIn() {
	...
	// AuthenticationController.get(). The server will yield a token, which we'll store in a global variable. If the
...
function onSignatureCompleted(signature) {
	...
	$.ajax({
		method: 'POST',
		url: '/api/authentication?token=' + token, // the token is guaranteed to be a URL-safe string
