...
public class AuthenticationController {
	...
	public String get(Model model, HttpServletResponse response) throws RestException {
		...
		// signWithRestPki() method on the Web PKI component (see file signature-form.js) and also to call the
		// completeWithWebPki() method on the post() method below (this should not be mistaken with the API access token).
		...
		// The token acquired above can only be used for a single authentication attempt. In order to retry the signature
		// it is necessary to get a new token. This can be a problem if the user uses the back button of the browser,
		// since the browser might show a cached page that we rendered previously, with a now stale token. To prevent this
	...
	public String post(
		...
		// templates/authentication.html). This method finalizes the authentication process, yielding a ValidationResults
